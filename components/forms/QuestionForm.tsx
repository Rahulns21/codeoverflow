...
const QuestionForm = () => {
    ...
    const handleCreateQuestion = (data: z.infer<typeof AskQuestion>) => {
        console.log(data);
    };
          ...
          <div>
            ...
            {tagsField.value.length > 0 && (
                ...
                className='flex-start mt-2.5 flex-wrap gap-2.5'>
                    {tagsField?.value?.map((tag: string) => <TagCard key={tag} _id={tag} name={tag} 
                    compact remove isButton handleRemove={() => handleTagRemove(tag)} />)}
                </div>
            )}
          </div>
          <FieldDescription className="body-regular mt-2.5 text-light-500">
